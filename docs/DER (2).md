...
O sistema utiliza uma arquitetura hibrida: MySQL como banco de dados principal (fonte da verdade) e MongoDB para cache e analytics (dados temporarios e flexiveis).

### Visao Geral das Collections

| Collection | Descricao | TTL | Indices | Proposito |
|------------|-----------|-----|---------|-----------|
| product_cache | Cache de produtos populares | 1 hora | product_id (unico), expires_at | Reduz carga no MySQL |
| order_events | Eventos de pedidos para analytics | - | timestamp, order_id | Analytics e relatorios |
| user_sessions | Sessoes ativas de usuarios | 7 dias | session_id, expires_at | Gerenciamento de sessoes |

### 1. product_cache

**Proposito:** Armazenar produtos frequentemente acessados em cache para reduzir queries no MySQL.

**Schema (Flexivel):**
```javascript
{
  "_id": ObjectId("..."),
  "product_id": 123,               // ID do produto no MySQL
  ...
  "views": 1523,                    // Contador de visualizacoes
  "last_updated": ISODate("2024-11-18T10:00:00Z"),
  "expires_at": ISODate("2024-11-18T11:00:00Z")  // TTL: expira em 1 hora
}
```

**Indices:**
- `{ product_id: 1 }` - Unico (busca rapida por produto)
- `{ expires_at: 1, expireAfterSeconds: 0 }` - TTL (remocao automatica)

**Operacoes Principais:**
- `cacheProduct()` - Insere/atualiza produto no cache
- `getCachedProduct()` - Recupera produto do cache
- `clearProductCache()` - Limpa cache de produto especifico

### 2. order_events

**Proposito:** Registrar eventos relacionados a pedidos para analytics e relatorios sem impactar transacoes principais.

**Schema (Flexivel):**
```javascript
{
  "_id": ObjectId("..."),
  "event_type": "order_placed",    // Tipos: order_placed, status_changed, order_cancelled
  "order_id": 12345,               // ID do pedido no MySQL
  "user_id": 789,                  // ID do usuario no MySQL
  "timestamp": ISODate("2024-11-18T10:30:00Z"),
  "data": {                         // Dados especificos do evento (schema flexivel)
    "items_count": 3,
    "total_value": 75.50,
    "payment_method": "pix",
    "category": "Lanches",
    "previous_status": "PENDENTE",  // Para eventos de mudanca de status
...
**Indices:**
- `{ timestamp: -1 }` - Ordenacao por data (mais recentes primeiro)
- `{ order_id: 1 }` - Busca eventos de um pedido especifico

**Operacoes Principais:**
- `logOrderEvent()` - Registra novo evento
- `getOrderEvents()` - Recupera eventos de um pedido

### 3. user_sessions

**Proposito:** Gerenciar sessoes ativas de usuarios com expiracao automatica.

**Schema (Flexivel):**
```javascript
{
  "_id": ObjectId("..."),
  "session_id": "session_abc123def456",  // ID unico da sessao
  "user_id": 789,                        // ID do usuario no MySQL
  "token": "eyJhbGciOiJIUzI1NiIsInR5...", // JWT token
  "created_at": ISODate("2024-11-18T10:00:00Z"),
  "expires_at": ISODate("2024-11-25T10:00:00Z"),  // TTL: 7 dias
  "last_activity": ISODate("2024-11-18T12:30:00Z"),
  "device_info": {                       // Informacoes do dispositivo (opcional)
    "type": "mobile",
    "os": "Android",
    "browser": "Chrome",
    "ip_address": "192.168.1.1"
...
**Indices:**
- `{ session_id: 1 }` - Busca rapida por sessao
- `{ expires_at: 1, expireAfterSeconds: 0 }` - TTL (remocao automatica)

**Operacoes Principais:**
- `cacheUserSession()` - Cria nova sessao
- `getUserSession()` - Recupera sessao ativa
- `removeUserSession()` - Remove sessao (logout)
...
|                                                             |
|  usuarios | produtos | pedidos | enderecos | categorias    |
|  itens_pedido | avaliacoes | cupons | log_pedidos          |
+-------------------------------------------------------------+
```
...
1. **Leitura de Produto:**
   ...
   - Backend verifica cache MongoDB (product_cache)
   - Se encontrado (cache hit): retorna imediatamente
   - Se nao encontrado (cache miss): busca no MySQL e armazena no cache

2. **Criacao de Pedido:**
   - Pedido criado no MySQL (fonte da verdade)
   - Evento registrado no MongoDB (order_events) de forma assincrona
   - Nao impacta performance da transacao principal

3. **Autenticacao:**
   - Login validado no MySQL
   - Sessao armazenada no MongoDB (user_sessions)
   - Token JWT retornado ao cliente
   - Expiracao automatica apos 7 dias (TTL)

**Vantagens da Arquitetura Hibrida:**
- MySQL garante integridade transacional (ACID)
- MongoDB oferece flexibilidade de schema e performance em cache
- Reducao de ~70% nas queries ao MySQL para dados frequentes
- Analytics nao impacta transacoes principais
- TTL automatico elimina dados temporarios sem intervencao manual
