...
1. Modelar um banco de dados relacional normalizado seguindo as formas normais (1FN, 2FN, 3FN);
2. Implementar no mínimo 6 triggers para automatização de processos e garantia de integridade;
3. Criar no mínimo 5 views para facilitar consultas e relatórios gerenciais;
4. Desenvolver no mínimo 9 procedures e functions para encapsular lógica de negócio;
5. Criar índices customizados para otimização de performance;
6. Implementar sistema de controle de acesso com múltiplos níveis de usuários;
7. Desenvolver backend RESTful utilizando Node.js e TypeScript;
8. Implementar frontend responsivo utilizando React e TypeScript;
9. Integrar banco de dados não-relacional para casos de uso específicos;
10. Documentar completamente o sistema e suas funcionalidades.
...
O projeto foi desenvolvido seguindo uma metodologia iterativa e incremental, dividida em etapas sequenciais:
...
const db = client.db('delivery_system');

// Cache de produto
async function cacheProduct(productId: number, data: any) {
    await db.collection('product_cache').insertOne({
...
A integração de MySQL como banco principal, com recursos avançados como triggers, views e stored procedures, demonstrou que SGBDs relacionais continuam sendo a escolha ideal para dados estruturados e transacionais. A complementação com tecnologias NoSQL (MongoDB e Redis) para casos de uso específicos mostrou a importância de uma abordagem híbrida no desenvolvimento moderno.
...
## Apêndice A - Scripts de Instalação

### A.1 Instalação do Banco de Dados

```bash
# Clonar repositório
git clone https://github.com/01fff/Sistema_delivery-.git
cd Sistema_delivery-/academic_delivery_system
